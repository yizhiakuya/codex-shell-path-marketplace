...
大多数用户可以直接把这个仓库链接发给 Codex，让 Codex 帮你安装插件：

```text
安装这个 Codex 插件，并使用 release binary 模式配置 Git Bash：
...
插件默认推荐使用 release binary 安装路径，用户不需要本地编译 Codex。安装完成后需要重启
Codex Desktop，并新开线程。
...
skill 会指导 Codex 运行插件内置脚本。推荐路径会使用 `-UseReleaseBinary`，也就是下载最新的
`codex-git-bash-windows-x86_64.zip` release 资产，而不是从源码构建。
